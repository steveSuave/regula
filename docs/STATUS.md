...
**Next**
- Trapezium macro — **decide the point story first** (PLAN doesn't pin one), then it's the same pattern. Leading candidate: taps A, B, C place the parallel sides' shared geometry, 4th tap picks D as a `PointOnObject` on the hidden parallel-to-AB through C (its analytic-parameter wart applies — D stays put when the line translates along itself). Alternative: a fixed-ratio D = C + k·(A − B) via a dialog like the segment-ratio tool. Update PLAN with the choice, then Phase 11 shortcuts.
