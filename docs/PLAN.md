...
| `X` `⇧ I` | Isosceles triangle *(planned, Phase 16 — plain `X I`/`X R` went to the Phase 18 quadrilaterals)* |
| `X` `⇧ R` | Right triangle *(planned, Phase 16)* |
| `X` `G` | Regular polygon *(planned, Phase 16 — random triangle/polygon stay menu-only)* |

Phase 16 chords are proposals; settle the final bindings when the tools land. (The Phase 15 transform chords above are settled — they live with the constructions under the `G` leader.)
